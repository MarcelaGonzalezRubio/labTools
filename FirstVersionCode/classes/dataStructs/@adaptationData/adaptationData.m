                       ...
                       w=adaptData.getParamInCond('TargetNexus',conditions{c});
                        ...
                        if nargin>4 && ~isempty(indivFlag) && indivFlag
                            ...
                            for s=1:length(subsToPlot)
                                ...
                                %to plot as dots
                                 Li{group}(s)=plot(x,indiv(group).(params{p}).(cond{c})(subInd,:),'o','MarkerSize',10,'MarkerEdgeColor',ColorOrder(subInd,:),'MarkerFaceColor',ColorOrder(subInd,:));
                                %to plot as lines
%                                 Li{group}(s)=plot(x,indiv(group).(params{p}).(cond{c})(subInd,:),LineOrder{group},'color',ColorOrder(subInd,:),'LineWidth',2);
                            ...
                            plot(x,y,'o','MarkerSize',10,'MarkerEdgeColor',[0 0 0],'MarkerFaceColor',[0.7 0.7 0.7].^group) 
%                             plot(x,y,'LineWidth',2,'color',[0.7 0.7 0.7].^group);
                        else
                            if Ngroups==1 && ~(size(params,1)>1) && nargin<6 
